...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        # N = X.shape[0]
        # input_size, hidden_size = self.params['W1'].shape
        # X1 = np.reshape(X, (N, input_size))
        # X2 = np.dot(X1, self.params['W1']) + self.params['b1']
        # X2[X2 < 0] = 0.
        # scores = np.dot(X2, self.params['W2']) + self.params['b2']
        ...
        # grads['W2'] = np.dot(X2.T, grad) + self.reg * self.params['W2']
        #
        # grad = np.dot(grad, self.params['W2'].T)
        # grad[X2 < 0] = 0
        #
        # grads['b1'] = np.sum(grad, axis=0)
        # grads['W1'] = np.dot(X1.T, grad) + self.reg * self.params['W1']

        dx, dw, db = affine_backward(grad, cache2)
        grads['W2'] = dw
        grads['b2'] = db
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(dim_size-1):
            self.params['W%s' % (i+1)] = np.random.normal(scale=weight_scale, size=(dims[i], dims[i+1]))
            self.params['b%s' % (i+1)] = np.zeros(dims[i+1])
        if self.use_batchnorm:
            for i in range(self.num_layers-1):
                self.params['gamma%s' % (i+1)] = np.ones(dims[i+1])
                self.params['beta%s' % (i+1)] = np.zeros(dims[i+1])
    ...
    def loss(self, X, y=None):
        ...
        if not self.use_batchnorm and not self.use_dropout:
            for i in range(self.num_layers-1):
                X, cache = affine_relu_forward(X, self.params['W%s' % (i+1)], self.params['b%s' % (i+1)])
                caches.append(cache)
